...
func TestCapacityHints(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "PodGroup release outside rejection path wakes conservatively", hintFn: podGroupHint, oldObj: podGroup("other", scheduling.PodGroupRunning), newObj: podGroup("other", scheduling.PodGroupCompleted), want: api.HintWakeup},
		...
		{name: "Pod deletion outside rejection path wakes conservatively", hintFn: podHint, oldObj: pod("other"), want: api.HintWakeup},
	}
