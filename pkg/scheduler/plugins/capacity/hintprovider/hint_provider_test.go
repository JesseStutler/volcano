...
func TestCapacityHints(t *testing.T) {
	...
	tests := []struct {
		...
		{name: "guarantee decrease outside scope wakes Job", hintFn: queueHint, oldObj: queueWithCPU("other", scheduling.QueueStateOpen, "", "12", ""), newObj: queueWithCPU("other", scheduling.QueueStateOpen, "", "4", ""), want: unschedulable.HintWakeup},
		{name: "deleting Queue with guarantee wakes Job", hintFn: queueHint, oldObj: queueWithCPU("other", scheduling.QueueStateOpen, "", "4", ""), want: unschedulable.HintWakeup},
		{name: "deleting unrelated Queue without guarantee is skipped", hintFn: queueHint, oldObj: queue("other", scheduling.QueueStateOpen, ""), want: unschedulable.HintSkip},
...
func TestCapacityEventsToRegister(t *testing.T) {
	want := []struct {
		...
		{event: fwk.ClusterEvent{Resource: unschedulable.QueueEvent, ActionType: fwk.Update | fwk.Delete}, hasHint: true},
...
func TestCapacityEventsToRegisterWithHierarchy(t *testing.T) {
	provider := NewCapacityHintProvider(true)
	events, err := provider.EventsToRegister(t.Context())
	if err != nil {
		t.Fatalf("EventsToRegister() error = %v", err)
	}
	if len(events) != 0 {
		t.Fatalf("EventsToRegister() returned %d events, want none", len(events))
	}

	job := api.NewJobInfo("job")
	jobCache := unschedulable.NewJobCache(unschedulable.DefaultMaxSkipDuration)
	jobCache.AddHintProvider("capacity", provider)
	jobCache.Record(job, []unschedulable.Rejection{{Plugin: "capacity", Source: unschedulable.RejectionEnqueue}})
	if got := jobCache.CachedRejections(job); len(got) != 0 {
		t.Fatalf("hierarchical Capacity rejection was cached: %v", got)
	}
}
