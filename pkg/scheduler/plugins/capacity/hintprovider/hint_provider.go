...
//
// It is intentionally stateless. Precise Queue filtering requires the shared
// incremental hierarchy/accounting model discussed in #5565; until that model
// exists, resource-release events wake conservatively because a sibling Queue
// may share quota with the rejected Job through an ancestor.
...
// additionally require an event that can release resource usage.
...
// quota. Without shared hierarchy/accounting state, a release outside the
// recorded Queue path may still come from a sibling sharing an ancestor.
func podGroupHint(_ *api.JobInfo, _ api.Rejection, oldObj, newObj any) (api.HintResult, error) {
	if newObj == nil {
		pg, ok := oldObj.(*api.PodGroup)
		if !ok || pg == nil {
			return api.HintWakeup, nil
		}
		if hintutil.PodGroupConsumesQuota(pg.Status.Phase) {
			return api.HintWakeup, nil
		}
		return api.HintSkip, nil
	}
...
// podHint wakes conservatively for a deletion or in-place scale-down. Without
// shared hierarchy/accounting state, the Pod's Queue alone cannot prove that a
// release is unrelated to the rejected Job's ancestor quota.
func podHint(_ *api.JobInfo, _ api.Rejection, oldObj, _ any) (api.HintResult, error) {
