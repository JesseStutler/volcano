...
// Hierarchical capacity rejections are not cached because precise wakeups need
// the Queue hierarchy and the Queue that caused the rejection.
...
// NewCapacityHintProvider creates a provider for the configured capacity mode.
func NewCapacityHintProvider(hierarchyEnabled bool) *CapacityHintProvider {
	return &CapacityHintProvider{hierarchyEnabled: hierarchyEnabled}
}
...
// queueWasCheckedForRejection reports whether queueName is in the Queue scope
// recorded for the rejected Job.
...
// queueHint wakes when a Queue change can loosen an admission or capacity
// constraint. Guarantee reductions are global because they reduce capacity's
// totalGuarantee even when the changed Queue is outside the Job's Queue path.
func queueHint(job *api.JobInfo, rejection unschedulable.Rejection, oldObj, newObj any) (unschedulable.HintResult, error) {
	...
	if newObj == nil {
		if len(oldQueue.Spec.Guarantee.Resource) != 0 {
			return unschedulable.HintWakeup, nil
		}
	...
	if hintutil.ResourceListIncreased(newQueue.Spec.Guarantee.Resource, oldQueue.Spec.Guarantee.Resource) {
		return unschedulable.HintWakeup, nil
	}
...
// quota from a Queue recorded in the capacity rejection.
