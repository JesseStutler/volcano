...
func (p *ResourceFitHintProvider) EventsToRegister(context.Context) ([]unschedulable.EventWithHint, error) {
	...
	return []unschedulable.EventWithHint{
		...
		{
			Event:       fwk.ClusterEvent{Resource: fwk.Node, ActionType: fwk.UpdateNodeAllocatable | fwk.UpdateNodeAnnotation},
...
func resourceFitNodeHint(job *api.JobInfo, rejection unschedulable.Rejection, oldObj, newObj any) (unschedulable.HintResult, error) {
	...
	if oldObj == nil {
		return newNodeFitHint(job, rejection, api.NewNodeAllocatable(newNode)), nil
	}
	...
	// resources increase.
	return allocatableIncreaseHint(job, rejection, api.NewNodeAllocatable(oldNode), api.NewNodeAllocatable(newNode)), nil
}
