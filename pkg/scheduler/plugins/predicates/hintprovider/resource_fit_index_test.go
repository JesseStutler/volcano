...
func TestResourceFitNodeGrowthEventKeysIncludeOversubscriptionIncrease(t *testing.T) {
	entries, err := (&ResourceFitHintProvider{}).EventsToRegister(context.Background())
	if err != nil {
		t.Fatalf("EventsToRegister() error = %v", err)
	}
	entry := findEvent(t, entries, fwk.Node, fwk.UpdateNodeAnnotation)

	oldNode := nodeWithOversubscriptionCPU("1", "0")
	oldNode.Name = "node-a"
	newNode := nodeWithOversubscriptionCPU("1", "1000")
	newNode.Name = "node-a"
	keys, err := entry.EventKeysFn(oldNode, newNode)
	if err != nil {
		t.Fatalf("EventKeysFn() error = %v", err)
	}
	assert.Equal(t, []unschedulable.HintKey{resourcefit.NodeGrowthKey("node-a", "cpu")}, keys)
}
