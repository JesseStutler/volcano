...
func TestResourceFitNodeHint(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "oversubscription annotation increase makes rejected task fit",
			oldObj: nodeWithOversubscriptionCPU("1", "0"),
			newObj: nodeWithOversubscriptionCPU("1", "1000"),
			want:   unschedulable.HintWakeup,
		},
		{
			name:   "new Node oversubscription makes rejected task fit",
			newObj: nodeWithOversubscriptionCPU("1", "1000"),
			want:   unschedulable.HintWakeup,
		},
	}
...
func nodeWithOversubscriptionCPU(cpu, oversubscriptionMilliCPU string) *v1.Node {
	node := nodeWithResources(cpu, "1Gi")
	node.Annotations = map[string]string{api.OversubscriptionCPU: oversubscriptionMilliCPU}
	return node
}
