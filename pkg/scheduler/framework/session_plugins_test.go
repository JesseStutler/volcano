...
type fakeUnschedulableCache struct {
	...
	recordedPriority map[api.JobID]int32
...
func (f *fakeUnschedulableCache) RecordUnschedulable(job *api.JobInfo, rejections []api.Rejection) {
	...
	if f.recordedPriority != nil {
		f.recordedPriority[job.UID] = job.Priority
	}
}
...
type closeOrderCache struct {
	cache.Cache
}

func (c *closeOrderCache) UpdateJobStatus(job *api.JobInfo, _, _, _ bool) (*api.JobInfo, error) {
	job.Priority = 99
	return job, nil
}

func (c *closeOrderCache) OnSessionClose() {}

func TestCloseSessionPublishesUnschedulableRecordAfterJobUpdate(t *testing.T) {
	job := api.NewJobInfo("job")
	job.Priority = 1
	job.PodGroup = &api.PodGroup{PodGroup: scheduling.PodGroup{
		ObjectMeta: metav1.ObjectMeta{Annotations: map[string]string{}},
	}}
	unschedulableCache := &fakeUnschedulableCache{
		recorded:         map[api.JobID][]api.Rejection{},
		recordedPriority: map[api.JobID]int32{},
	...
	ssn := &Session{
		...
		cache:                        &closeOrderCache{},
		unschedulableCache:           unschedulableCache,
		unschedulableJobCacheEnabled: true,
		jobRejections:                make(map[api.JobID]map[rejectionKey]*rejectionAggregate),
		PodGroupOldState: &api.PodGroupOldState{
			Status:      map[api.JobID]scheduling.PodGroupStatus{job.UID: {}},
			Annotations: map[api.JobID]map[string]string{job.UID: {}},
		},
	}
	...
	if got := unschedulableCache.recordedPriority[job.UID]; got != 99 {
		t.Fatalf("recorded Job priority = %d, want post-update priority 99", got)
	}
}
