...
func (sc *SchedulerCache) forgetUnschedulableJobForPod(pod *v1.Pod) {
	if pod == nil {
		return
	}
	...
	sc.unschedulableCache.ForgetUnschedulable(schedulingapi.JobID(fmt.Sprintf("%s/%s", pod.Namespace, groupName)))
}
...
func (sc *SchedulerCache) AddPod(obj interface{}) {
	...
	sc.forgetUnschedulableJobForPod(pod)
...
func (sc *SchedulerCache) UpdatePod(oldObj, newObj interface{}) {
	...
	if terminalRelease || hasSpecificSchedulingEvent {
		sc.forgetUnschedulableJobForPod(newPod)
	}
	if terminalRelease {
