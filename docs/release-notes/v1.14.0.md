...
### Scalable Multi-Scheduler with Dynamic Node Sharding (Alpha)
...
**Alpha Feature Notice**: This feature is currently in alpha stage. The NodeShard CRD API structure and the underlying sharding concepts are actively evolving. 

**Key Capabilities**:

- **Dynamic Node Sharding Strategies**: Compute dynamic candidate node pools based on various policies. Currently supports sharding by CPU utilization, with an extensible design to support more policies in the future.
